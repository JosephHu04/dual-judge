...
VLLM_BASE_URL = "http://192.168.253.95:8013/v1"
VLLM_MODEL = "qwen35-4b"
...
logger.info("LLM 已初始化: %s (vLLM 服务器, 工具数=%d)", VLLM_MODEL, len(ALL_TOOLS))
...
def should_continue(state: State) -> str:
    """
    ReAct 循环控制器。
    有 tool_calls → 执行工具。但单次请求最多调 5 次工具，防止无限循环。
    """
    ...
    if tool_count >= 5:
        logger.warning("工具调用已达上限(%d次)，强制结束", tool_count)
        return "__end__"
...
def invoke_agent_structured(message: str, session_id: str = "default") -> dict:
    ...
    try:
        ...
        reply = _strip_thinking(result["messages"][-1].content)
        ...
        logger.error("会话[%s] 工具调用超限", session_id)
        return {"response_text": "抱歉，处理您的请求时遇到了一些问题，请稍后再试或联系前台（电话：0000）。", "session_id": session_id, "tool_calls": []}
    except Exception as e:
        logger.error("会话[%s] 异常: %s", session_id, str(e), exc_info=True)
        return {"response_text": "非常抱歉，系统暂时遇到了一些问题。请致电前台（电话：0000），我们的工作人员会立即为您处理。", "session_id": session_id, "tool_calls": []}
...
def _strip_thinking(text: str) -> str:
    """过滤 Qwen3 思考模式：去掉 </think> 之前的所有内容"""
    if "</think>" in text:
